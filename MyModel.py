...
class MyModel(nn.Module):
	def __init__(self, lr, lr_decay, C , n_class, m, s, test_step, mixedprec, **kwargs):
		...
		# self.speaker_encoder = conformer().cuda()
		...
		# self.speaker_loss    = ARMSoftmax(n_class = n_class, m = m, s = s).cuda()
      
		self.optim           = torch.optim.Adam(self.parameters(), lr = lr, weight_decay = 2e-5)
		# self.optim           = torch.optim.SGD([{"params": self.parameters(), "initial_lr": lr}], lr=lr)
		self.scheduler       = torch.optim.lr_scheduler.StepLR(self.optim, step_size = test_step, gamma=lr_decay)
		# self.scheduler       = torch.optim.lr_scheduler.ExponentialLR(self.optim, gamma=0.94, last_epoch=150)
