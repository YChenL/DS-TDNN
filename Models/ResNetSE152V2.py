...
class ResNetSE(nn.Module):
    def __init__(self, block, layers, num_filters, nOut, encoder_type='ASP', n_mels=80, log_input=True, **kwargs):
        ...
        self.attention = nn.Sequential(
            nn.Conv1d(4*num_filters[3] * 5, 128, kernel_size=1),
            nn.ReLU(),
            nn.BatchNorm1d(128),
            nn.Conv1d(128, 4*num_filters[3] * 5, kernel_size=1),
            nn.Softmax(dim=2),
            )
...
def MainModel(nOut=256, **kwargs):
    ...
    model = ResNetSE(SEBottleneck, [3, 8, 36, 3], num_filters, nOut, **kwargs)
    return model
