...
class DS_TDNN_alter(nn.Module):

    def __init__(self, C, uniform_init=True, if_sparse=True):

        super(DS_TDNN_alter, self).__init__()
        self.sparse=True
        ...
        # TDNN layer
        self.llayer1 = LocalBlock(C, kernel_size=3, scale=4, dilation=1) #默认8 8 8 C=1024; 尝试4 6 8 C=960
        self.llayer2 = LocalBlock(C, kernel_size=3, scale=4, dilation=2)
        self.llayer3 = LocalBlock(C, kernel_size=3, scale=4, dilation=3)
         
        #global modules
        self.glayer1 = GlobalBlock_MAC(C, T=200, dropout=0.3,  K=4)
        self.glayer2 = GlobalBlock_MAC(C, T=200, dropout=0.1,  K=4)

        self.layer4 = nn.Conv1d(5*C, 1536, kernel_size=1)
        # ASP
    ...
    def forward_(self, x, aug=None):
        ...
        # apply MFA
        lx1 = self.llayer1(x)
        gx1 = self.glayer1.forward_(lx1+x)
        lx2 = self.llayer2(gx1+lx1+x)
        gx2 = self.glayer2.forward_(lx2+gx1+lx1+x)  
        lx3 = self.llayer3(gx2+lx2+gx1+lx1+x)
        
        x = self.layer4(torch.cat((lx1, gx1, lx2, gx2, lx3),dim=1))
        x = self.relu(x)
    ...
    def forward(self, x, aug=None):
        ...
        # apply MFA
        lx1 = self.llayer1(x)
        gx1 = self.glayer1(lx1+x)
        lx2 = self.llayer2(gx1+lx1+x)
        gx2 = self.glayer2(lx2+gx1+lx1+x)  
        lx3 = self.llayer3(gx2+lx2+gx1+lx1+x)
